...
(define (eval-expo rho expr v)
  (fresh ()
    (absento 'in expr)
    (matche expr
      ((quote ,datum) (not-in-rhoo 'quote rho) (== datum v))
      ((list . ,e*) (not-in-rhoo 'list rho) (eval-listo rho e* v))
      (,x (symbolo x) (lookupo `(,x : ,v) rho))
      ((lambda (,x) ,e) (not-in-rhoo 'lambda rho) (== `((lambda (,x) ,e) in ,rho) v))
      ((,e1 ,e2)
       (fresh (x e rho1 v2)
         (eval-expo rho e1 `((lambda (,x) ,e) in ,rho1))
         (eval-expo rho e2 v2)
         (eval-expo `((,x : ,v2) . ,rho1) e v))))))

(define (evalo expr v) (eval-expo '() expr v))

(define (eval-listo rho expr v)
  (matche (expr v)
    ((() ()))
    (((,e . ,e*) (,ve . ,ve*)) (eval-expo rho e ve) (eval-listo rho e* ve*))))
...
(define (not-in-rhoo x rho)
  (matche rho
    (())
    (((,y . ,v) . ,rho1) (=/= y x) (not-in-rhoo x rho1))))
